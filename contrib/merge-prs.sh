...
if [[ "$1" == "setup" ]]; then
    ...
    if git worktree list --porcelain | ggrep --silent prunable; then
        echo "You have stale git worktrees, please either fix them or run 'git worktree prune'."
        exit 1
    fi
    git worktree list --porcelain | ggrep --silent "${WORKTREE}" || git worktree add "${WORKTREE}" --force --no-checkout --detach
    echo
...
do
    ...
    PR_ID=$(echo "$line" | ggrep -o -P "#\d+")
    REPO=$(echo "$line" | ggrep -o -P '\S+(?=\s*#\d+)')
    ...
    if [[ "$1" == "analyze" ]]; then
        ...
        NUM_FILES=$(ggrep -c "CONFLICT" "$MERGE_FILE")
        NUM_CONFLICTS=$(ggrep -c "<<<<<<<" "$DIFF_FILE")
